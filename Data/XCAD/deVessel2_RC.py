...
class DeVessel():
    ...
    def __init__(self , config):
        ...
        if self.config["mode"]=="G":
            print("高斯模糊")
        elif self.config["mode"]=="DEC_ND":
            print("双椭圆约束_无细节")
        elif self.config["mode"]=="RC":
            print("单矩形框约束")
        elif self.config["mode"]=="DEC":# DEC
            print("双椭圆约束")
        else:# DEC
            print("mode 参数不合法!")
    ...
    def process(self, inPath, outPath):
        ...
        # shape (605, 700)
        # exit(0)
        if self.config["mode"]=="DEC_ND":
            img_deVessel3=self.deVessel_DEC_ND(img)
        elif self.config["mode"]=="RC":
            img_deVessel3=self.deVessel_RC(img)
        elif self.config["mode"]=="DEC":
            img_deVessel3=self.deVessel_DEC(img)
        elif self.config["mode"]=="G":
            img_deVessel3=self.deVessel_G(img)
    ...
    def deVessel_DEC_ND(self,img):
        ...
        # mutate the amplitude part of source with target
        # amp_src_ = low_freq_mutate_np(amp_src, amp_trg, L=L)
        pha_src = np.fft.fftshift( fft_src_np, axes=(-2, -1) )
        _, h, w = pha_src.shape # h=512 w=512   
        for i in range(h):
            for j in range(w):
                x=(i-h/2)/self.width
                ...
                if k>self.alpha and k<self.beta : 
                    pha_src[0,i,j]=0#a_src2[0,i,j]
        fft_src_ = np.fft.ifftshift(pha_src, axes=(-2, -1))
    ...
    def deVessel_G(self, img0):
        def gaussian_kernel(size, sigma=1.0):
            
            # 创建一个二维网格
            x, y = np.mgrid[-size//2 + 1:size//2 + 1, -size//2 + 1:size//2 + 1]
            g = np.exp(-((x**2 + y**2) / (2.0 * sigma**2)))
            #这个sigma标准差不是这组像素数值的标准差，而是高斯函数图像的标准差、描述了图像的水平拉伸程度
            return g / g.sum()  # 归一化，使核的总和为1
        ...
        # img_FDA_guassian = cv2.GaussianBlur(img0, (13, 13), 0)
        if True:
            img1 = cv2.GaussianBlur(img0, (55, 55), 10)
        else:
            kernel=gaussian_kernel(13,10)
            img1 = apply_gaussian_blur(img0, kernel)
        # 3.添加点噪声
        # noise_map = np.random.uniform(-5, 5, img_FDA_guassian.shape)
        if False:
            ...
            img2 = img1 + noise_map
            result = np.clip(img2, -np.pi, np.pi)
        result = np.clip(img1, 0, 255)
        # arr_expanded = np.expand_dims(result, axis=0)
        # print(result.shape)
        return result #arr_expanded
