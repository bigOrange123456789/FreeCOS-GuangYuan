...
class Trainer():
    ...
    def train(self,epoch,isFirstEpoch):
        ...
        for idx in pbar:
            ...
            optimizer_l.zero_grad() # 默认情况下，梯度会被累加。因此需要清零梯度。
            ...
            loss_seg,loss_D_tar,loss_D_src,loss_adv,loss_ce,loss_dice,loss_contrast_sum,loss_adv_sum = \
                self.processBatch(minibatch,unsup_minibatch,damping,isFirstEpoch)
            lr = self.step(current_idx)
    ...
    def processBatch(self,minibatch,unsup_minibatch,damping,isFirstEpoch):
        Segment_model = self.Segment_model
        predict_Discriminator_model = self.predict_Discriminator_model
        ...
        loss_ce = 0.1 * criterion_bce(pred_sup_l,
                                      gts)  # 根据预测结果和标签计算CE损失 # retinal是5 XCAD是0.1 crack是5 # For retinal :5 For XCAD:0.1 5 for crack
        loss_dice = criterion(pred_sup_l, gts)  # 根据预测结果和标签计算Dice损失
        pred_target, sample_set_unsup, flag_un = Segment_model(unsup_imgs, mask=None, trained=True,
                                                               fake=False)  # mask影响正负样本的获取
        ...
        loss_adv = (loss_adv_target * damping) / 4 + loss_dice + loss_ce + weight_contrast * (
            loss_contrast)  # 对抗+监督+对比
        ...
        # retain_graph=False：不保留用于计算梯度的计算图（graph）。
        ...
        # .detach()：“分离”出一个副本，这个副本在自动微分过程中不会被用于梯度计算。
        ...
        (loss_D_src+loss_D_tar).backward(retain_graph=False)  # 计算判别器参数的梯度,并累加到网络参数的.grad属性中
        # 反向传播（Backpropagation）：计算每个参数的梯度。

        return loss_seg,loss_D_tar,loss_D_src,loss_adv,loss_ce,loss_dice,loss_contrast_sum,loss_adv_sum
