...
class Img():
    ...
    def getLight(self):
        ...
        # thickness=
        light0 = 1
        delta = 0.1 # delta = random.uniform(0.01, 0.02) # 0.02 # 0.01
    ...
    def save(self, output_path = "output_image.png", output_path2 = "output_image2.png"): # 保存图像 
        ...
        if connected and ratio>2.5/100.: 
            if True:
                pic = Image.fromarray(self.image, mode='L')  # 'L' 表示灰度图像 
                pic.save(output_path) 
            light=self.getLight()
            pic = Image.fromarray(255.*light, mode='L')  # 'L' 表示灰度图像 
            pic.save(output_path2) 
    ...
    def draw(self,new_length,width,x,y,theta):
        ...
        x=h/2+x
        y=w/2+y
        new_length=new_length+width #width/2.
        """
            在numpy数组上绘制一条线段。
            参数:
                image: numpy数组,表示图像。
            ...
            for j in range(w):
                ...
                if projection_length <= width / 2:
                    ...
                    if (pixel_to_start_x * direction_x + pixel_to_start_y * direction_y >= 0 and
                        ...
                        self.thickness[i,j] = max(self.thickness[i,j],thickness)
